...
const WordPreview = ({ wordId }) => {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        DictionaryService.getWordPreview(wordId)
            .then(({ data }) => {
                if (data) previewCache.set(wordId, data);
                setPreview(data);
            })
...
const ConfigurationGameView = ({ onActivityCreated, redirectPath }) => {
    ...
    useEffect(() => {
        ...
        Promise.all([
            GameService.getById(editId).then(r => {
                if (!r.success) throw new Error(r.error || 'No se pudo cargar el juego.');
                return r.data;
            }),
    ...
    const handleSubmit = async () => {
        ...
        try {
            const res = isEditMode
                ? await GameService.update(editId, dto)
